...
const schema = z.object({
  ...
  origin_airport_code: z.string().length(3, '3 lettres').toUpperCase(),
  destination_city: z.string().min(2, 'Requis'),
  destination_airport_code: z.string().length(3, '3 lettres').toUpperCase(),
  departure_date: z.string().min(1, 'Requis'),
  flight_number: z.string().optional(),
  total_kg: z.coerce.number().min(1, 'Min 1 kg'),
  max_kg_per_package: z.coerce.number().min(0.5, 'Min 0.5 kg'),
  price_per_kg: z.coerce.number().min(0.5, 'Min 0.5€'),
})
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ padding: '24px 20px 100px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 16, border: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <Input
              label={t.carrier.date_label}
              type="date"
              error={errors.departure_date?.message}
              {...register('departure_date')}
            />
            <Input
              label={t.carrier.flight_label}
              placeholder="AF502"
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 16, border: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <Input
              label={t.carrier.kg_label}
              type="number"
              placeholder="20"
              step="0.5"
              error={errors.total_kg?.message}
              {...register('total_kg')}
            />
            <Input
              label={t.carrier.max_kg_label}
              type="number"
              placeholder="5"
              step="0.5"
              error={errors.max_kg_per_package?.message}
              {...register('max_kg_per_package')}
            />
            <Input
              label={t.carrier.price_label}
              type="number"
              placeholder="3"
              step="0.5"
