...
export default function CarrierPage() {
  ...
  const { data: pendingBookings = [], isLoading: loadingBookings } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data.filter((b: any) =>
        b.status === 'pending' && myTrips.some((t: any) => t.id === b.trip_id)
      )
    },
  ...
  // Onboarding — pas encore transporteur
  if (!user?.is_carrier) {
    return (
      <div style={{ minHeight: '100vh', background: '#FBFBFF', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        ...
        {[
          ...
          { num: '2', label: 'Complétez votre vérification KYC' },
        ...
        {user?.kyc_status !== 'verified' && (
          <div style={{ background: '#FFF8E1', border: '1px solid #FFE082', borderRadius: 12, padding: '10px 16px', marginTop: 16, marginBottom: 16, maxWidth: 320, width: '100%' }}>
            <p style={{ fontSize: 13, color: '#F59E0B', fontWeight: 500 }}>⚠️ {t.carrier.onboarding_kyc}</p>
          </div>
        )}
    ...
    <div style={{ background: '#FBFBFF', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '20px 20px 80px' }}>
        ...
        <button
          ...
          style={{ width: '100%', background: '#fff', border: `2px dashed ${BORDER}`, borderRadius: 16, padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer', marginBottom: 24, transition: 'all 0.2s' }}
