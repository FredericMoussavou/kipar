...
class Booking(Base):
    ...
    package_request_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("package_requests.id"), nullable=True, index=True)
    ...
    awaiting_receiver_reminded_24h: Mapped[bool] = mapped_column(Boolean, default=False)
    awaiting_receiver_reminded_36h: Mapped[bool] = mapped_column(Boolean, default=False)
    ...
    booking_flat_fee_amount: Mapped[float] = mapped_column(Float, default=1.5)
    ...
    cancellation_review_status: Mapped[str | None] = mapped_column(String(20), nullable=True)  # None | pending_review | justified | unjustified
    carrier_penalty_due: Mapped[float] = mapped_column(Float, default=0.0)
    cancellation_evidence_urls: Mapped[list | None] = mapped_column(JSON, nullable=True)
    cancellation_justification: Mapped[str | None] = mapped_column(String(1000), nullable=True)
